...
def main():
    ...
    data_time = sys.argv[2] if len(sys.argv) > 2 else datetime.utcnow().isoformat(timespec="milliseconds") + "Z"
    ...
    print("数据源：Yahoo Finance / 用户提供数据 / 其他来源")
    print("覆盖标的：" + "，".join(f"{name}（{code}）" for name, code in DEFAULT_SYMBOLS))
    print()
    print("今日结论先行：")
    print("- 市场主判断：")
    print("- 仓位建议：")
    ...
    for name, code in DEFAULT_SYMBOLS:
        print(f"{name}（{code}）")
        print()
